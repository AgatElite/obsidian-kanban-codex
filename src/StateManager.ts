...
export class StateManager {
  ...
  shouldRefreshForMetadataChange(file: TFile): boolean {
    if (!this.state || this.file === file || !this.getSetting('metadata-keys')?.length) {
      return false;
    }

    return this.state.children.some((lane) => {
      return lane.children.some((item) => {
        return item.data.metadata.file?.path === file.path;
      });
    });
  }
  ...
  setState(state: Board | ((board: Board) => Board), shouldSave: boolean = true) {
    try {
      ...
      if (didSettingsChange && newSettings) {
