...
import {
  archiveOnCheckString,
  archiveString,
  settingsToCodeblock,
  showCheckboxesString,
} from '../common';
    ...
    if (child.type === 'heading') {
      ...
      const list = getNextOfType(root.children, index, 'list', (child) => {
        ...
        if (child.type === 'paragraph') {
          ...
          if (childStr === 'Show card checkboxes') {
            showCheckboxes = true;
            return true;
          }

          if (childStr === 'Archive checked cards') {
            archiveOnCheck = true;
            return true;
          }
        }
...
function laneToMd(lane: Lane) {
  ...
  if (lane.data.showCheckboxes) {
    lines.push(showCheckboxesString);
  }

  if (lane.data.archiveOnCheck) {
    lines.push(archiveOnCheckString);
  }
